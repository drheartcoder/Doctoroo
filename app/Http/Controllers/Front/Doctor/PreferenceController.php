...
{
    public function __construct(  DoctorModel        $doctor_model,
                                  UserModel          $user_model,
                                  DoctorPreferencesModel $prefernces_model)
    {
        $this->arr_view_data            = [];
        ...
        $this->DoctorPreferencesModel   = $prefernces_model;
    ...
    */
    ...
    {
          $arr_preference   = $arr_days = $arr_data = [];
          $arr_days         = $this->arr_days;

          $obj_preference   = $this->DoctorPreferencesModel->where('user_id','=',$this->user_id)
                                                           ->whereHas('doctor_details',function(){

                                                           })
                                                           ->with(['doctor_details'=>function($q){
          ...
          if(isset($arr_preference) && sizeof($arr_preference)>0)
          {
                  foreach ($arr_preference as $key=>$prefernces) 
                  {
                      ...
                      $arr_data['noti_message']               = $prefernces['doctor_details']['noti_message'];
                      $arr_data['noti_new_patient']           = $prefernces['doctor_details']['noti_new_patient'];
                      $arr_data['noti_new_booking']           = $prefernces['doctor_details']['noti_new_booking'];
                      $arr_data['noti_ans_a_question']        = $prefernces['doctor_details']['noti_ans_a_question'];
                      $arr_data['noti_accept_aust_patients']  = $prefernces['doctor_details']['noti_accept_aust_patients'];
          ...
          $this->arr_view_data['page_title']       = 'Preferences';
        ...
        if(sizeof($arr_days)>0)
        {
               foreach ($arr_days as $day_key => $day) 
               {
                     ...
                     $arr_time_data['to_time']   = convert_12_to_24($request->input($small_case_day_slug.'_to_time'));
                      ...
                      $is_exist_count = $this->DoctorPreferencesModel->where('user_id','=',$this->user_id)
                      ...
                      if($is_exist_count>0)
                      {
                            ...
                            $arr_update['status']     = $arr_time_data['status'];
                             ...
                             $obj_update = $this->DoctorPreferencesModel
                                                ->where('user_id','=',$this->user_id)
                                                ->where('day',$small_case_day_slug)->update($arr_update);
                      }   
                      else
                      {
                            $obj_create = $this->DoctorPreferencesModel->create($arr_time_data);
              ...
              $obj_doctor      = $this->DoctorModel->where('user_id',$this->user_id)->first();
              if($obj_doctor)
              {
                   $obj_doctor = $obj_doctor->update($arr_pref_notification);
              }
              else
              {
                   $arr_pref_notification['user_id']             =  $this->user_id;
                   $obj_doctor = $this->DoctorModel->create($arr_pref_notification);
              }
