    ...
    <div class="mar300  has-header minhtnor doctor-patient-list-main">
        <div class="consultation-tabs ">
            <ul class="tabs tabs-fixed-width">
                <li class="tab">
                    <a href="{{$module_url_path}}/patients/myown_patients" onclick="location.href = '{{$module_url_path}}/patients/myown_patients'"><span><img src="{{url('/')}}/public/doctor_section/images/patient-icon.png" alt="icon" class="tab-icon"/> </span> My Own Patients </a>
                </li>
                <li class="tab">
                    <a class="active" onclick="location.href = '{{$module_url_path}}/patients/doctoroo_patients'" style="cursor: pointer;"> <span><img src="{{url('/')}}/public/doctor_section/images/doctoroo-patient.png" alt="icon" class="tab-icon"/> </span> doctoroo Patients</a>
                </li>                
            </ul>
        ...
        <div id="medical" class="tab-content medi patient-list-block doctor-patient-list-main">
            <div class="patient-list-heading">
                <span class="patient-list-title">
                    Previously seen doctoroo Patients
                </span>                
            </div>
                        ...
                        <div class="table-cell center-align">Patient Details</div>
                            ...
                            @foreach($patients_arr['data'] as $val)
                                <div class="table-row content-row-table">
                                    ...
                                    @php
                                        $src="";
                                        if(isset($val['patient_user_details']['profile_image']) && !empty($val['patient_user_details']['profile_image']) && File::exists($profile_img_base_path.$val['patient_user_details']['profile_image']))
                                        {
                                    ...
                                    @endphp
                                        ...
                                        <span class="patient-name-block">{{isset($val['patient_user_details']['title']) ? $val['patient_user_details']['title'] : ''}} {{isset($val['patient_user_details']['title']) ? $val['patient_user_details']['first_name'] : ''}} {{isset($val['patient_user_details']['last_name']) ? $val['patient_user_details']['last_name'] : ''}}</span>
                                    </div>                        
                                    <div class="table-cell patient-type">{{ isset($val['patient_info']['type']) && $val['patient_info']['type'] =='doctoroo' ? 'Doctoroo' : '-' }}</div>
                                    <div class="table-cell patient-birthdate">
                                        {{isset($val['patient_info']['date_of_birth']) ? date("d/m/Y", strtotime($val['patient_info']['date_of_birth'])) : ''}}
                                    </div>
                                    ...
                                    <div class="table-cell patient-details view-details-btn"><a href="{{ url('/') }}/doctor/patients/details/{{ base64_encode($val['patient_user_details']['id']) }}">View details</a></div>
                                </div>            
                            @endforeach
                </div>
                @else
                    <h5 class="center-align no-data" style="color: #184059;">No Records found</h5>
                @endif
