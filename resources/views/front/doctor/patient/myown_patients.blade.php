        ...
        <div class="searchform">
            ...
            <div class="other">
                <div class="input-field">
                    ...
                    <input type="hidden" name="patient_type" id="patient_type" value="myown">
                </div>
            </div>
            ...
            <div class="other">
                <div class="input-field">
                    ...
                    <input id="entitlement_id" name="entitlement_id" type="text">
...
<div class="mar300  has-header minhtnor doctor-patient-list-main">
    <div class="consultation-tabs ">
        <ul class="tabs tabs-fixed-width">
            <li class="tab">
                <a class="active" onclick="location.href = '{{$module_url_path}}/patients/myown_patients'"  style="cursor: pointer;"><span><img src="{{url('/')}}/public/doctor_section/images/patient-icon.png" alt="icon" class="tab-icon"/> </span> My Own Patients </a>
            </li>
            <li class="tab">
                <a href="{{$module_url_path}}/patients/doctoroo_patients" onclick="location.href = '{{$module_url_path}}/patients/doctoroo_patients'"> <span><img src="{{url('/')}}/public/doctor_section/images/doctoroo-patient.png" alt="icon" class="tab-icon"/> </span> doctoroo Patients</a>
            </li>
        </ul>
    ...
    <div id="patient" class="tab-content medi patient-list-block doctor-patient-list-main">
        <div class="patient-list-heading">
            <span class="patient-list-title">
                My Own Patients
            </span>
            <a href="{{$module_url_path}}/patients/add_patient" class="add-new-patient-btn">
                + Add a new Patient
            </a>
        </div>
                ...
                <div class="table ">
                    ...
                    <div class="table-row content-row-table">
                        <div class="table-cell patient-name">
                            @php $src=""; 
                                if(isset($val['userinfo']['profile_image']) && !empty($val['userinfo']['profile_image']) && File::exists($profile_img_base_path.$val['userinfo']['profile_image']))
                                 {
                                     $src = $profile_img_public_path.$val['userinfo']['profile_image'];
                                 }
                                else
                                 { 
                            ...
                            <span class="patient-name-block">{{isset($val['userinfo']['title']) ? $val['userinfo']['title'] : ''}} {{isset($val['userinfo']['first_name']) ? $val['userinfo']['first_name'] : ''}} {{isset($val['userinfo']['last_name']) ? $val['userinfo']['last_name'] : ''}}</span>
                        </div>
                        <div class="table-cell patient-type">
                            {{ isset($val['type']) && $val['type'] =='myown' ? 'My Own' : '-' }}
                        </div>
                        <div class="table-cell patient-birthdate">
                            {{isset($val['date_of_birth']) ? date("d/m/Y", strtotime($val['date_of_birth'])) : ''}}
                        </div>
                        <div class="table-cell patient-gender">
                            @php
                                if(isset($val['gender']))
                                {
                                    if($val['gender']=='F')   
                        ...
                        <div class="table-cell patient-details view-details-btn">
                            <a href="{{ url('/') }}/doctor/patients/details/{{ base64_encode($val['userinfo']['id']) }}">View details</a>
                        </div>
                    </div>
                    @endforeach
                </div>
                @else
                <h5 class="center-align no-data" style="color: #184059;">No Records found</h5> @endif
