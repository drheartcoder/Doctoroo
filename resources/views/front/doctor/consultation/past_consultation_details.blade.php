                        ...
                        <div class="col l5 m5 s5">
                            <div class="consultation-id-block">
                                <div class="consultation-id-label">
                                    ...
                                    {{isset($past_consultation_arr['patient_user_details']['title']) ? $past_consultation_arr['patient_user_details']['title'] : ''}}
                                    {{isset($past_consultation_arr['patient_user_details']['first_name']) ? $past_consultation_arr['patient_user_details']['first_name'] : ''}} {{isset($past_consultation_arr['patient_user_details']['last_name']) ? $past_consultation_arr['patient_user_details']['last_name'] : ''}} 
                                    @elseif(isset($past_consultation_arr['familiy_member_info'])) {{isset($past_consultation_arr['familiy_member_info']['first_name']) ? $past_consultation_arr['familiy_member_info']['first_name'] : ''}} {{isset($past_consultation_arr['familiy_member_info']['last_name']) ? $past_consultation_arr['familiy_member_info']['last_name'] : ''}} 
                                    @endif
                                </div>
                                <div class="consultation-id-content">
                                    Consulting Patient
                                ...
                                <div class="col l6 m6 s6 col-payment-invoice">
                                    ...
                                    @foreach($past_consultation_arr['invoice_info'] as $invoice_data)
                                        @php
                                            ...
                                            $total_amount = add_gst($pay_amount);
                                        @endphp
...
<script type="text/javascript">
    ...
    $('#btn_download_pdf').click(function(){
        $.ajax({
               url:"{{$module_url_path}}/past_consultation_details/download/{{isset($past_consultation_arr['id']) ? base64_encode($past_consultation_arr['id']) : ''}}",
               type:'get',
               success:function(response){
                  ...
                  if(response!='')
                  {
                    ...
                    $.ajax({
                       url:"{{$module_url_path}}/past_consultation_details_generate_pdf",
                       type:'post',
                       data:{'arr_data' : response,'_token' : _token},
                       
                       success:function(response){
                            pdf_url = "{{$module_url_path}}/past_consultation_details_generate_pdf";
