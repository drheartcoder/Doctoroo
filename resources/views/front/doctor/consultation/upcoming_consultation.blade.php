        ...
        <div class="z-depth-3 round-box">
            ...
            <div class="transactions-table table-responsive paitent-list-table">
                ...
                <div class="table consultation_lists" id="">
                    ...
                    <?php /*
                    @if($upcoming_consultation_arr['data'])
                                ...
                                <div class="table-row content-row-table">
                                    <div class="table-cell transaction-id">
                                        ...
                                        <span class="patient-name-block">
                                            ...
                                            {{isset($val['patient_user_details']['first_name']) ? $val['patient_user_details']['first_name'] : ''}} 
                                            {{isset($val['patient_user_details']['last_name']) ? $val['patient_user_details']['last_name'] : ''}}
                                    ...
                                    <div class="table-cell transaction-date">{{isset($val['consultation_datetime']) ? date("h:i a D, M d Y", strtotime($val['consultation_datetime'])) : ' '}}</div>
                                    <div class="table-cell transaction-price">Doctoroo</div>
                                    <div class="table-cell transaction-desciption"><span class="description">{{isset($val['booking_status']) ? $val['booking_status'] : 'Not found'}}</span></div>
                                    <div class="table-cell transaction-status view-details-btn"><a href="{{$module_url_path}}/upcoming_consultation_details/{{ base64_encode($val['id']) }}">View details</a></div>
                                </div>
                            @endforeach
                    @endif 
                    @if(sizeof($upcoming_consultation_arr['data'])==0)
                        <div class="table-row  hidden-xs">
                            <div class="no-data" style="color: #184059;">
                                No data found
                            </div>
