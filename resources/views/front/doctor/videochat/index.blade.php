...
<head>
   <meta charset="utf-8" />
    ...
    <style>
        ...
        .text-chat-section p{height: 360px; overflow: scroll;}
        .text-chat-section form{margin: 0;}
...
<body  style="background-color:#184059;">
        ...
        <div class="video-call-main">
            <div class="call-action-btns">
                ...
                <div class="end-call-btn">
                    <a href="javascript:void(0);" id="close_call" onclick="end_call()"><i class="fa fa-phone"></i></a>
                </div>
  ...
  <script>
        interval = setInterval(function(){ check_patient_active() }, 1000);
        ...
        function check_patient_active()
        {

          var token = "<?php echo csrf_token(); ?>";
          var booking_id = document.getElementById("booking_id").value;
          $.ajax({
            url   : "{{ url('/') }}/doctor/video/check_patient_active",
            type : "POST",
            dataType: 'json',
            ...
            success : function(res){
              if(res)
              {
                  if(res.call_status == 'end'){
                    hideProcessingOverlay();
                    swal({ title: "Patient has ended this call...",   
                         text: "",  
                         type: "warning",   
                  ...
                  if(res.call_status == 'reject'){
                    hideProcessingOverlay();
                    swal({ title: "Patient has rejected this call...",
                         text: "",  
                         type: "warning",   
                         showCancelButton: false,   
                         confirmButtonColor: "#184059",
        ...
        /*function close_call(){
          ...
          var booking_id = document.getElementById("booking_id").value;
          $.ajax({
              url: '{{ url("/") }}/doctor/video/update_video_call_end_status',
              type: 'POST',
              //dataType: 'json',
              data: {
                  _token: _token,
        ...
        function update_call_time(){
            ...
            var booking_id             = document.getElementById("booking_id").value;
            var url                    = '{{ url("/") }}/doctor/video/update_video_call_time';
            var _token = "<?php echo csrf_token(); ?>";
            $.ajax({
                url:url,
                type:'POST',
                     ...
                     if(last_stored_time == data){
                           if(last_stored_time_cnt<=10){
                            document.getElementById('last_stored_time_cnt').value = parseInt(last_stored_time_cnt) + parseInt(1);
                              ...
                              swal({ title: "Unfortunataly Patient's call disconnected....",   
                               text: "",  
                               type: "warning",   
                               showCancelButton: false,   
                               confirmButtonColor: "#184059",  
