...
@section('main_content')
    ...
    <div class="header bookhead">
        ...
        <div class="backarrow "><a href="{{ url('/') }}/patient/booking" class="center-align"><i class="material-icons">&#xE5CB;</i></a></div>
        ...
        <div class="container">
            <div class="available-now">
                ...
                <ul class="collection">
                    @if(isset($available_doctor_today) && !empty($available_doctor_today))
                        ...
                        @foreach($available_doctor_today as $doc_data)
                            @if($counter <= $show_records) 
                                ...
                                if(substr($doc_data->fee_start_time, 11) <= $current_time && substr($doc_data->fee_end_time, 11) >= $current_time)
                                {
                                    $consult_fee = isset($doc_data->total_patient_fee_for_four_min) ? '$'.number_format($doc_data->total_patient_fee_for_four_min, 2, '.', '') : '';
                                }
                                else
                                {
                                    ...
                                    $consult_fee = '$'.number_format($get_consult_fee->total_patient_fee_for_four_min, 2, '.', '');
                                }
                                @endphp
                                <?php $get_avl_doc_data = \DB::table('users')->where('id',$doc_data->user_id)->first(); 
                                ...
                                <li class="collection-item avatar ">
                                    <a class="valign-wrapper get_date" href="{{ url('/') }}/patient/booking/available_doctor/{{ base64_encode($doc_data->user_id) }}/{{ base64_encode($today_date) }}" data-date="{{ $today_date }}">
                                        ...
                                        <div class="doc-detail  left"><span class="title alv_doc<?php echo $doc_data->user_id; ?>">{{ $doc_data->title.' '.$doc_data->first_name.' '.$doc_data->last_name }}</span>
                                            <script type="text/javascript">
                                                /*var alv_doc_id           = "{{$doc_data->user_id}}";
                                                var alv_doc_first_name   = "{{$doc_data->first_name}}"; 
                                                var alv_doc_last_name    = "{{$doc_data->last_name}}"; 
                                                var alv_doc_title        = "{{$doc_data->title}}"; 
                                                ...
                                                var decrpyt_alv_doc_first_name   = key.decrypt(alv_doc_first_name).toString();
                                                var decrpyt_alv_doc_last_name    = key.decrypt(alv_doc_last_name).toString();
                                                var dr_name = alv_doc_title+' '+decrpyt_alv_doc_first_name+' '+decrpyt_alv_doc_last_name;
                                                $('.alv_doc'+alv_doc_id).html(dr_name);*/
                                            </script>
                    ...
                    @else
                        <li class="collection-item avatar ">
                            <p class="no-data">{{ "Unfortunately no doctors are currently available" }}</p>
                        </li>
                    @endif
                </ul>
                @if(sizeof($available_doctor_today) > $show_records)
                    <div class="show-all-wraper">
                        <a href="{{$module_url_path}}/show_available_doctors/today" class="btn show-all-btn">Show All</a>
                    </div>
                @endif
                ...
                <ul class="collection ava-tomorrow">
                    @if(isset($available_doctor_tomorrow) && !empty($available_doctor_tomorrow))
                        @php $counter = 1; @endphp
                        @foreach($available_doctor_tomorrow as $next_doc_data)
                            @if($counter <= $show_records) 

                            @php
                            // check listisng image
                            if ( isset($next_doc_data->profile_image) && !empty($next_doc_data->profile_image) )
                            {
                                $profile_images = $doctor_image_url.$next_doc_data->profile_image;
                            ...
                            $consult_fee = isset($next_doc_data->total_patient_fee_for_four_min) ? '$'.number_format($next_doc_data->total_patient_fee_for_four_min, 2, '.', '') : '';
                            if($consult_fee == '')
                            {
                                $get_consult_fee = DB::table('doctor_fees')->select('total_patient_fee_for_four_min')
                                                                           ->where('doctor_id', $next_doc_data->user_id)
                                                                           ->orderBy('total_patient_fee_for_four_min', 'DESC')
                                                                           ->first();
                            ...
                            @endphp
                            <?php $get_next_doc_data = \DB::table('users')->where('id',$next_doc_data->user_id)->first(); 
                                  if(isset($get_next_doc_data->dump_id) && $get_next_doc_data->dump_id != ""){ $dump_id = $get_next_doc_data->dump_id; } else { $dump_id = ""; }
                                  if(isset($get_next_doc_data->dump_session) && $get_next_doc_data->dump_session != ""){ $dump_session = $get_next_doc_data->dump_session; } else { $dump_session = ""; }
                            ?>
                            <li class="collection-item avatar ">
                                <a class="valign-wrapper get_date" href="{{ url('/') }}/patient/booking/available_doctor/{{ base64_encode($next_doc_data->user_id) }}/{{ base64_encode($tomorrow_date) }}" data-date="{{ $tomorrow_date }}">
                                    <div class="image-avtar left"> <img src="{{ $profile_images }}" alt="" class="circle" />
                                        @if($next_doc_data->is_online == 1)
                                            <span class="onlinenew"></span>
                                        @else
                                            <span class="online"></span>
                                        @endif
                                    </div>
                                    <div class="doc-detail left "><span class="title next_doc<?php echo $next_doc_data->user_id; ?>">{{ $next_doc_data->title.' '.$next_doc_data->first_name.' '.$next_doc_data->last_name }}</span>
                                        <p class="doctor-price"><strong>Consult Fee :</strong> {{ $consult_fee }}</p>
                                    </div>
                                    <script type="text/javascript">
                                        /*var next_doc_id          = "{{$next_doc_data->user_id}}";
                                        var next_doc_first_name  = "{{$next_doc_data->first_name}}"; 
                                        var next_doc_last_name   = "{{$next_doc_data->last_name}}"; 
                                        var next_doc_title       = "{{$next_doc_data->title}}"; 
                                        var card_id              = "{{ $dump_id }}"
                                        var userkey              = "{{ $dump_session }}";
                                        var VIRGIL_TOKEN         = "{{ env('VIRGIL_TOKEN') }}";
                                        var api                  = virgil.API(VIRGIL_TOKEN);
                                        var key                  = api.keys.import(userkey);
                                        var decrpyt_next_doc_first_name   = key.decrypt(next_doc_first_name).toString();
                                        var decrpyt_next_doc_last_name    = key.decrypt(next_doc_last_name).toString();
                                        var dr_name = next_doc_title+' '+decrpyt_next_doc_first_name+' '+decrpyt_next_doc_last_name;
                                        $('.next_doc'+next_doc_id).html(dr_name);*/
                                    </script>
                            ...
                            @endif @php $counter += 1; @endphp
                        @endforeach
                    @else
                        <li class="collection-item avatar ">
                            <p class="no-data">{{ "Unfortunately no doctors are available" }}</p>
                        </li>
                    @endif

                </ul>
                @if(sizeof($available_doctor_tomorrow) > $show_records)
                    <div class="show-all-wraper">
                        <a href="{{$module_url_path}}/show_available_doctors/tomorrow" class="btn show-all-btn">Show All</a>
                    </div>
                @endif
            </div>
