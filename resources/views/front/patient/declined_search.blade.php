        ...
        <form method="GET" id="search_doctor_form" name="search_doctor" action="{{ url('/patient') }}/declined_consultation/search_declined_consultation">
            <div class="searchform">
                ...
                <div class="side-footer search-botom">
                    <a href="{{ url('/') }}/patient/declined_consultations" class="left search-btn">CLEAR</a>
            ...
            <div id="test3" class="col s12 tab-content medi">
                ...
                @if(isset($declined_consult_arr['data']) && !empty($declined_consult_arr['data']))
                    <ul class="collection brdrtopsd">
                        @foreach($declined_consult_arr['data'] as $declined_consult_data)
                            
                            <?php
                                $consult_datetime = convert_utc_to_userdatetime($patient_id, "patient", $declined_consult_data["consultation_datetime"]);
                            ?>

                            <li class="collection-item valign-wrapper">
                                <div class="left wid100"><small>{{ date("l d F, Y",strtotime($consult_datetime)).' '.date("h:i a",strtotime($consult_datetime)) }}</small>
                                    <span class="title">{{ $declined_consult_data["doctor_user_details"]["title"].' '.$declined_consult_data["doctor_user_details"]["first_name"].' '.$declined_consult_data["doctor_user_details"]["last_name"] }}</span>
                                </div>
                                <div class="right posrel">
                                <a href="#" data-activates='dropdown{{ $declined_consult_data["id"] }}' class="dropdown-button"><i class="fa fa-th-list" aria-hidden="true"></i></a></div>
                                <ul id='dropdown{{ $declined_consult_data["id"] }}' class='dropdown-content doc-rop rightless'>
                                    <li><a href="{{ url('/patient') }}/declined_consultation/details/{{base64_encode($declined_consult_data["id"])}}">View Consultation Details</a></li>
