...
@section('main_content')
    ...
    <div class="header consultation-detailshead z-depth-2 bookhead">
        <div class="backarrow"><a href="{{url('/')}}/patient/past_consultations" class="center-align"><i class="material-icons">&#xE5CB;</i></a></div>
        <!-- <h1 class="main-title left-align">Past Consultation Details</h1> -->
    </div>
    ...
    <div class="mar300 has-header has-footer">
        ...
        <div class="consultation-details">
            <div class="sub-header  z-depth-2">
                    ...
                    <div class="col s6 m6 l6 left-align">
                        <div class="data">
                            <label class="white-text">
                                @if(isset($past_consult_arr['familiy_member_info']))
                                       @php $val = $past_consult_arr['familiy_member_info']; @endphp
                                       {{isset($val['first_name']) ? $val['first_name'] : ''}}
                                       {{isset($val['last_name']) ? $val['last_name'] : ''}}
                                @elseif(isset($past_consult_arr['patient_user_details']))
                                     @php $val = $past_consult_arr['patient_user_details']; @endphp
                        ...
                        <div class="data">
                            <label class="white-text">{{isset($past_consult_arr['doctor_user_details']['title']) ? $past_consult_arr['doctor_user_details']['title'] : ''}} {{isset($past_consult_arr['doctor_user_details']['first_name']) ? $past_consult_arr['doctor_user_details']['first_name'] : ''}} {{isset($past_consult_arr['doctor_user_details']['last_name']) ? $past_consult_arr['doctor_user_details']['last_name'] : ''}}</label>
                            <small>Doctor</small></div>
                    </div>

                    <?php $consult_datetime = convert_utc_to_userdatetime($patient_id, "patient", $past_consult_arr["consultation_datetime"]); ?>

                    <div class="col s6 m6 l6 right-align">
                        <label class="date white-text">{{ date("d M Y",strtotime($consult_datetime)) }}</label>
                        <label class="consId white-text"><span>Consultation ID:</span> {{isset($past_consult_arr['consultation_id']) ? $past_consult_arr['consultation_id'] : ''}}</label>
                        
                        <?php $consult_id = isset($past_consult_arr['id']) ? base64_encode($past_consult_arr['id']) : ''; ?>

                        <a class="view-invoice white-text" href="{{ url('/patient') }}/past_consultation/invoice/{{$consult_id}}">View Invoice <span><i class="material-icons">&#xE873;</i></span></a>
                    </div>
                </div>
                <div class="row detInfo mrtp">
                    <div class="col s12 m12 l12 left-align">
                        ...
                        <div class="data">
                            <input type="hidden" value="{{isset($past_consult_arr['card_number']) ? $past_consult_arr['card_number'] : ''}}" id="card_no">
                            @php
                                if(isset($past_consult_arr['card_number'])) 
                                {
                                     $temp_no = $past_consult_arr['card_number'];
            ...
            <div class="data-content">
                <ul class="collapsible" data-collapsible="expandable">
                    <li>
                        <div class="collapsible-header waves-effect waves-light">Doctor Notes <i class="material-icons right">expand_more</i></div>
                        <div class="collapsible-body">
                            @php 
                                    $notes = isset($past_consult_arr['consultation_notes']['notes']) && !empty($past_consult_arr['consultation_notes']['notes']) ? $past_consult_arr['consultation_notes']['notes'] : 'Not notes added';
                                @endphp
                                 <span>{{ substr($notes,0,200) }}</span> 
                                 @if( !empty(substr($notes,200)))
                                     <span class="more-content" style="display: none" id="notes_more">
                                        {{ substr($notes,200) }}
                                    </span>
                                    <a class="expand-more-btn green-text" id="notes_more_btn"> Read More </a><a class="close-more-btn green-text" style="display: none" id="notes_less">Close </a> 
                                @endif
                        ...
                        <div class="collapsible-body">
                            <ul class="purpose-consultation">
                                @php
                                    ...
                                    if(isset($past_consult_arr['consultation_for'])) { $consultation_for = explode(',',$past_consult_arr['consultation_for']); }
                                @endphp
                                    ...
                                    @foreach($consultation_for as $key=>$purpose)
                                        @if($purpose != "")    
                                            ...
                                            <script type="text/javascript">
                                                ...
                                                var card_id                = "{{ isset($past_consult_arr['patient_user_details']['dump_id']) && !empty($past_consult_arr['patient_user_details']['dump_id']) ? $past_consult_arr['patient_user_details']['dump_id'] : '' }}"
                                                var userkey                = "{{ isset($past_consult_arr['patient_user_details']['dump_session']) && !empty($past_consult_arr['patient_user_details']['dump_session']) ? $past_consult_arr['patient_user_details']['dump_session'] : '' }}";
                    ...
                    <li>
                        ...
                        <div class="collapsible-body">    
                            <p class="dec_symptoms"><!-- {{ isset($past_consult_arr['symptoms']) && !empty($past_consult_arr['symptoms']) ? $past_consult_arr['symptoms'] : ''}} --></p>
                        </div>
                            <script type="text/javascript">
                                $(document).ready(function(){
                                    var patient_id             = "{{ isset($past_consult_arr['patient_user_id']) && !empty($past_consult_arr['patient_user_id']) ? $past_consult_arr['patient_user_id'] : '' }}";
                                    var symptoms               = "{{ isset($past_consult_arr['symptoms']) && !empty($past_consult_arr['symptoms']) ? $past_consult_arr['symptoms'] : '' }}"; 
                                    var card_id                = "{{ isset($past_consult_arr['patient_user_details']['dump_id']) && !empty($past_consult_arr['patient_user_details']['dump_id']) ? $past_consult_arr['patient_user_details']['dump_id'] : '' }}"
                                    var userkey                = "{{ isset($past_consult_arr['patient_user_details']['dump_session']) && !empty($past_consult_arr['patient_user_details']['dump_session']) ? $past_consult_arr['patient_user_details']['dump_session'] : '' }}";
                                    var VIRGIL_TOKEN           = "{{env('VIRGIL_TOKEN')}}";
                                    var api                    = virgil.API(VIRGIL_TOKEN);
                    ...
                    <li>
                        ...
                        <div class="collapsible-body">
                            <ul class="collection brdrtopsd ">
                                @if(isset($past_consult_arr['consultation_documents']) && !empty($past_consult_arr['consultation_documents']))
                                    @foreach($past_consult_arr['consultation_documents'] as $key => $val)
                                                ...
                                                <div class="row">
                                                    <div class="col l9 m9 s8">
                                                        <div class="valign-wrapper pres">
                                                            ...
                                                            <a target="_blank" class="document_show_{{$key}}"><p class="green-text">{{$val['document']}}</p></a>
                                                        </div>
                                                    </div>
                                                    <div class="col l3 m3 s4 right actionnew">
                                                        <a  class="circle btn-floating bluedoc-bg z-depth-1 right document_file_{{$key}}" download><i class="material-icons">&#xE2C4;</i></a>
                                                        <a   class="circle btn-floating bluedoc-bg z-depth-1 right dec_document_file_{{$key}}" target="_blank"><i class="material-icons">remove_red_eye</i></a>
                                                    </div>
                                                </div>
                                            </li>
                                            <!-- Decrypt Images -->
                                        <script type="text/javascript">
                                         ...
                                         var dumpSessionId = '{{isset($past_consult_arr["patient_user_details"]["dump_session"])?$past_consult_arr["patient_user_details"]["dump_session"]:""}}';
                                         var dumpId        = '{{isset($past_consult_arr["patient_user_details"]["dump_id"])?$past_consult_arr["patient_user_details"]["dump_id"]:""}}';
                                         if(dumpSessionId!='')
                                         {
                                                ...
                                                xhr.onload = function( e ) {
                                                  ...
                                                  fileReader.onload = function ()
                                                  {
                                                    ...
                                                    if(img=='' && imageUrl=='')
                                                    {
                                                        ...
                                                        img.href      = imageUrl;
                                                        $(".document_file_"+innerkey).attr('href',imageUrl);
                                                        $(".dec_document_file_"+innerkey).attr('href',imageUrl);
                                                        $(".document_show_"+innerkey).attr('src',imageUrl);
                                                    }
                                                  }
                                                };
                                                xhr.send();
                                    ...
                                    @endforeach
                                @else
                                    <span class="green-text">No document and certificate Uploaded</span>
                                @endif
                    ...
                    <li>
                        <div class="collapsible-header waves-effect waves-light">Disputes <i class="material-icons right">expand_more</i></div>
                        <div class="collapsible-body">
                                @if(isset($past_consult_arr['disputes']) && !empty($past_consult_arr['disputes']))
                                    @foreach($past_consult_arr['disputes'] as $dispute)
                                        <div>
                                            <div class="payment-image-box">
                                                <img src="{{url('/')}}/public/doctor_section/images/handshake.svg" alt="" style="width: 50px;">
                                            </div>
        ...
        <a class="waves-effect waves-light futbtn" href="{{url('/')}}/patient/past_consultations">close</a>
