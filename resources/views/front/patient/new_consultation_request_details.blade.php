...
@section('main_content')
    ...
    <div class="header consultation-detailshead z-depth-2 bookhead">
        <div class="backarrow"><a href="{{url('/')}}/patient/dashboard" class="center-align"><i class="material-icons">&#xE5CB;</i></a></div>
            ...
            <div class="sub-header  z-depth-2">
                <div class="row detInfo">
                    <div class="col s6 m6 l6 left-align">
                        <div class="data">
                            <label class="white-text">
                                @if(isset($new_consult_arr['familiy_member_info']))
                                       @php $val = $new_consult_arr['familiy_member_info']; @endphp
                                       {{isset($val['first_name']) ? $val['first_name'] : ''}}
                                       {{isset($val['last_name']) ? $val['last_name'] : ''}}
                                @elseif(isset($new_consult_arr['patient_user_details']))
                                     @php $val = $new_consult_arr['patient_user_details']; @endphp
                        ...
                        <div class="data">
                            <label class="white-text">{{isset($new_consult_arr['doctor_user_details']['title']) ? $new_consult_arr['doctor_user_details']['title'] : ''}} {{isset($new_consult_arr['doctor_user_details']['first_name']) ? $new_consult_arr['doctor_user_details']['first_name'] : ''}} {{isset($new_consult_arr['doctor_user_details']['last_name']) ? $new_consult_arr['doctor_user_details']['last_name'] : ''}}</label>
                            <small>Doctor</small></div>

                    </div>
                    <?php
                        if($new_consult_arr["consultation_datetime"] != '' && isset($new_consult_arr["consultation_datetime"]))
                        {
                            $consult_datetime = convert_utc_to_userdatetime($patient_id, "patient", $new_consult_arr["consultation_datetime"]);
                        }
                        else
                        {
                            $consult_datetime = '';
                    ...
                    <div class="col s6 m6 l6 right-align">
                        ...
                        <label class="consId white-text"><span>Consultation ID:</span> {{isset($new_consult_arr['consultation_id']) ? $new_consult_arr['consultation_id'] : ''}}</label>
                    </div>
                </div>
                <div class="row detInfo mrtp">
                    <div class="col s12 m12 l12 left-align">
                        <div class="data left">
                            <label class="white-text">
                                @php 
                                    ...
                                    if(isset($new_consult_arr['consultation_charge']))
                                    {
                                         $consultation_charge = $new_consult_arr['consultation_charge'];
                        ...
                        <div class="data">
                            <input type="hidden" value="{{isset($new_consult_arr['card_number']) ? $new_consult_arr['card_number'] : ''}}" id="card_no">
                            @php
                                if(isset($new_consult_arr['card_number'])) 
                                {
                                     $temp_no = $new_consult_arr['card_number'];
            ...
            <div class="data-content">
                <ul class="collapsible" data-collapsible="expandable">
                        ...
                        <div class="collapsible-body">
                            <ul class="purpose-consultation">
                                @php
                                    ...
                                    if(isset($new_consult_arr['consultation_for'])) { 
                                      $consultation_for = explode(',',$new_consult_arr['consultation_for']); 
                                    }
                                @endphp
                                    ...
                                    @foreach($consultation_for as $key=>$purpose)
                                        @if($purpose != "")    
                                            ...
                                            <script type="text/javascript">
                                                ...
                                                var card_id                = "{{ isset($new_consult_arr['patient_user_details']['dump_id']) && !empty($new_consult_arr['patient_user_details']['dump_id']) ? $new_consult_arr['patient_user_details']['dump_id'] : '' }}"
                                                var userkey                = "{{ isset($new_consult_arr['patient_user_details']['dump_session']) && !empty($new_consult_arr['patient_user_details']['dump_session']) ? $new_consult_arr['patient_user_details']['dump_session'] : '' }}";
                    ...
                    <li>
                        ...
                        <div class="collapsible-body">    
                            <p class="dec_symptoms"><!-- {{ isset($new_consult_arr['symptoms']) && !empty($new_consult_arr['symptoms']) ? $new_consult_arr['symptoms'] : ''}} --></p>
                        </div>
                            <script type="text/javascript">
                                $(document).ready(function(){
                                    var patient_id             = "{{ isset($new_consult_arr['patient_user_id']) && !empty($new_consult_arr['patient_user_id']) ? $new_consult_arr['patient_user_id'] : '' }}";
                                    var symptoms               = "{{ isset($new_consult_arr['symptoms']) && !empty($new_consult_arr['symptoms']) ? $new_consult_arr['symptoms'] : '' }}"; 
                                    var card_id                = "{{ isset($new_consult_arr['patient_user_details']['dump_id']) && !empty($new_consult_arr['patient_user_details']['dump_id']) ? $new_consult_arr['patient_user_details']['dump_id'] : '' }}"
                                    var userkey                = "{{ isset($new_consult_arr['patient_user_details']['dump_session']) && !empty($new_consult_arr['patient_user_details']['dump_session']) ? $new_consult_arr['patient_user_details']['dump_session'] : '' }}";
                                    var VIRGIL_TOKEN           = "{{env('VIRGIL_TOKEN')}}";
                                    var api                    = virgil.API(VIRGIL_TOKEN);
                                ...
                                @foreach($health_images_arr as $key =>  $val)
                                    @if($val['health_image'] !='' && File::exists($patient_uploads_url.$val['health_image']))
                                        ...
                                        <script type="text/javascript">
                                         ...
                                         var dumpSessionId = '{{isset($new_consult_arr["patient_user_details"]["dump_session"])?$new_consult_arr["patient_user_details"]["dump_session"]:""}}';
                                         var dumpId        = '{{isset($new_consult_arr["patient_user_details"]["dump_id"])?$new_consult_arr["patient_user_details"]["dump_id"]:""}}';
                                         if(dumpSessionId!='')
                                         {
