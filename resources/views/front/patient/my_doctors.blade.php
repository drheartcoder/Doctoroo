            ...
            <div id="test4" class="col s12 tab-content medi">
                <div class="available-now martp">
                    @if(isset($consult_arr) && !empty($consult_arr))
                        <ul class="collection">
                            @foreach($consult_arr as $consult_data)
                            
                            @php
                                // check listisng image
                                if ( isset($consult_data['doctor_user_details']['profile_image']) && !empty($consult_data['doctor_user_details']['profile_image']) )
                                {
                                    $profile_images = $doctor_image_url.$consult_data['doctor_user_details']['profile_image'];
                                ...
                                $consult_date = isset($consult_data['doctor_availability'][0]['date'])?$consult_data['doctor_availability'][0]['date']:'';
                                $doc_id = isset($consult_data['doctor_user_details']['id'])?$consult_data['doctor_user_details']['id']:'';

                                $consult_datetime = convert_utc_to_userdatetime($patient_id, "patient", $consult_data["consultation_datetime"]);
                            @endphp

                            <li class="collection-item avatar ">
                               <div class="valign-wrapper">
                                <div class="image-avtar left"> <img src="{{ $profile_images }}" alt="" class="circle" />
                                    @if($consult_data['doctor_user_details']['is_online'] == 1)
                                        <span class="onlinenew"></span>
                                    @else
                                        <span class="online"></span>
                                    @endif
                                </div>
                                <div class="doc-detail  left"><span class="title">{{ $consult_data["doctor_user_details"]["title"].' '.$consult_data["doctor_user_details"]["first_name"].' '.$consult_data["doctor_user_details"]["last_name"] }}</span>
                                ...
                                @if($consult_data['doctor_availability'] != null)
