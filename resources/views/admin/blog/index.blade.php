    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box-content">
          ...
          <div class="btn-group">
          <a href="{{ $module_url_path.'/create'}}" class="btn btn-primary btn-add-new-records"  title="Add CMS">Add Blog</a> 
          </div>         
            <div class="btn-group">
               ...
               <tbody>
                 @foreach($arr_blog as $blog)
                  <tr>
                     ...
                     <td>{{ isset($blog['title'])?$blog['title']:'' }} </td>                      
                     <td>               
                     @if(isset($blog['image']) && !empty($blog['image']))
                      <img src="{{$blog_img_url_path}}/{{$blog['image']}}" width="30px" height="30px">
                     @endif
                     </td>
                     <td>
                       {{isset($blog['postedby'])?$blog['postedby']:''}}
                     </td>
                     <td>
                      @if($blog['status']=="Active")
                        <a href="{{ $module_url_path.'/deactivate/'.base64_encode($blog['id']) }}" class="btn btn-success">Active</a>
                        @else
                        <a href="{{ $module_url_path.'/activate/'.base64_encode($blog['id']) }}" class="btn btn-danger">Inactive</a>
                        @endif
                     </td>
                     <td> 
                        ...
                        &nbsp;  
                        <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip" href="{{ $module_url_path.'/delete/'.base64_encode($blog['id'])}}" 
