          ...
          <div class="table-responsive" style="border:0">
            ...
            <input type="hidden" name="multi_action" value="" />
              ...
              <table class="table table-advance"  id="table1" >
               ...
               <tbody>
                  @foreach($arr_info as $arr_faq)
                  <tr>
                     <td> 
                        <input type="checkbox" 
                           name="checked_record[]"  
                           value="{{ base64_encode($arr_faq['id']) }}" /> 
                     </td>
                     <td> 
                        {{ isset($arr_faq['question'])?$arr_faq['question']:'' }}  
                     </td>
                     <td> {{ isset($arr_faq['answer'])?$arr_faq['answer']:'' }} </td>
                     <td>
                        @if($arr_faq['status']=="Active")
                        <a href="{{ $module_url_path.'/deactivate/'.base64_encode($arr_faq['id']) }}" class="btn btn-success">Active</a>
                        @else
                        <a href="{{ $module_url_path.'/activate/'.base64_encode($arr_faq['id']) }}" class="btn btn-danger">Inactive</a>
                        @endif
                     </td>
                     <td> 
                        <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip" href="{{ $module_url_path.'/edit/'.base64_encode($arr_faq['id']) }}"  title="Edit">
                        <i class="fa fa-edit" ></i>
                        </a>  
                        &nbsp;  
                        <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip" href="{{ $module_url_path.'/delete/'.base64_encode($arr_faq['id'])}}" 
